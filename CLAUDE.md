...
- **Display**: `Emulator::draw()` calls `renderer::render_frame()` at 160×144 (scaled 3× by the window layer). Frame rate is capped near ~59.7 fps.
...
61 tests currently pass across `tests/cpu.rs`, `tests/ops.rs`, and renderer unit tests in `src/renderer.rs`. Keep them green.
